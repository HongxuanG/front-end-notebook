...
const MyPromise = /** @class */ (function() {
    ...
    var promise2 = new MyPromise((resolve, reject) => {
      if (_this.status === Status.FULFILLED) {
        setTimeout(() => {
          try {
            ...
            const x = onfulfilledFn(_this.value)
      ...
      if (_this.status === Status.REJECTED) {
        setTimeout(() => {
          try {
            //  获取到 x，然后与要返回的 Promise 产生联系
            const x = onrejectedFn(_this.reason)
            resolvePromise(promise2, x, resolve, reject)
          }
          catch (e) {
            reject(e)
  ...
  MyPromise.prototype.finally = function(onfinally) {
    return this.then((value) => {
      ...
      return MyPromise.resolve(typeof onfinally === 'function' ? onfinally() : onfinally).then(() => {
        return value
      })
    }, (reason) => {
      return MyPromise.resolve(typeof onfinally === 'function' ? onfinally() : onfinally).then(() => {
  ...
  MyPromise.all = function(values) {
    return new MyPromise((resolve, reject) => {
      ...
      const resolveResult = function(value, index, done) {
        resultArr[index] = value
        doneArr[index] = true
        if (done && doneArr.every((item) => {
          return item
        })) {
          resolve(resultArr)
        }
      ...
      const _loop_2 = function(i) {
        ...
        if (isPromise(value)) {
          value.then((value) => {
            ...
            doneArr[i] = true
            if (cur.done && doneArr.every((item) => {
              return item
            })) {
              reslove(resultArr)
            }
